...
tier_c_minimal_config() {
  ...
  # Try to preserve port, channel config from current/broken config or backups
  local gateway_port=""
  local channel_json=""

  # Extract from current (broken) config first
  if [ -f "$OPENCLAW_CONFIG" ]; then
...
" 2>/dev/null || echo "")
    channel_json=$(python3 -c "
import json
try:
    d = json.load(open('$OPENCLAW_CONFIG'))
    ch = d.get('channels',{})
    if ch: print(json.dumps(ch))
except: pass
" 2>/dev/null || echo "")
  fi

  # If no channel config from broken config, try backups
  if [ -z "$channel_json" ] && [ -d "$BACKUP_DIR" ]; then
    local bak
    for bak in $(ls -1t "$BACKUP_DIR"/openclaw-*.json 2>/dev/null | head -3); do
      channel_json=$(python3 -c "
import json
try:
...
" 2>/dev/null || echo "")
  ...
  if [ "$DRY_RUN" = true ]; then
    log "[DRY-RUN] Would generate minimal config (port=$gateway_port, channels=$([ -n "$channel_json" ] && echo 'preserved' || echo 'none'))"
    return 0
  fi

  save_broken_config

  # Generate minimal safe config matching OpenClaw schema
  # Fixes: gateway.mode=local required, no gateway.host, agents.defaults not
  # agents.main, no unknown root keys (_generated etc), preserve channels
  python3 -c "
import json, os
...
# Restore channel config (Telegram etc) if available
channel_raw = '''$channel_json'''
if channel_raw.strip():
    try:
        config['channels'] = json.loads(channel_raw)
    except: pass
...
" 2>/dev/null
  ...
  log "Generated minimal safe config (port=$gateway_port, mode=local, no plugins/skills)"
