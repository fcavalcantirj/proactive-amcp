...
SYSTEMD_ENV_FILE="$SYSTEMD_ENV_FILE" python3 << 'EOF'
import json
import os
import shutil
...
if not secrets_file:
    # Fallback to reading from marker
    import sys
    secrets_file = sys.argv[1] if len(sys.argv) > 1 else ""

# Read from environment variable set by bash
secrets = json.load(open(os.environ.get("SECRETS_FILE_PATH", secrets_file)))
backup_dir = os.environ.get("BACKUP_DIR", "/tmp/amcp-backup")
systemd_env_file = os.environ.get("SYSTEMD_ENV_FILE", os.path.expanduser("~/.config/openclaw/env"))
...
for secret in secrets:
    ...
    for target in targets:
        ...
        if kind == "file":
            ...
            name = target.get("name", key)
            # Write to EnvironmentFile that systemd reads
...
# Write systemd EnvironmentFile if any
if systemd_env_lines:
    ...
    # Write new env file (systemd format: KEY="value", no export)
    ...
    print(f"[SYSTEMD] Services to reload: {', '.join(services_to_reload)}")
    
    # Write services list for bash to reload
...
export SYSTEMD_ENV_FILE="$SYSTEMD_ENV_FILE"

# Re-run with proper env
python3 << PYEOF
import json
import os
import shutil
