...
cleanup() {
  ...
  send_telegram "🛑 *proactive-amcp Ralph* - Runner Stopped
...
echo -e "${MAGENTA}${BOLD}║                                                                   ║${NC}"
echo -e "${MAGENTA}${BOLD}║   🚀  PROACTIVE-AMCP - RALPH CONTINUOUS RUNNER                             ║${NC}"
echo -e "${MAGENTA}${BOLD}║                                                                   ║${NC}"
...
send_telegram "🟢 *proactive-amcp Ralph* - Runner Started
...
while true; do
  ...
  send_telegram "🚀 *proactive-amcp Ralph* - Batch #${batch_count} Starting
...
🔄 Running ${BATCH_SIZE} iterations..."
  ...
  if [ "$api_error" = true ]; then
    ...
    if [ -n "$error_detail" ]; then
      send_telegram "🚨 *proactive-amcp Ralph* - API Error
...
Phase 1 done! Ready for Phase 2 when you are. 🚀"
      ...
      send_telegram "🎉 *proactive-amcp PHASE 3 COMPLETE!* 🎉
